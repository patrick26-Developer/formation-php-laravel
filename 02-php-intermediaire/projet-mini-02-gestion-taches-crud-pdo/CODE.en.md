# Complete Source Code

This project's entire code already exists in this repository — click a file to open it, copy it as-is into your own Laravel project by following [INSTALLATION.md](INSTALLATION.en.md).

## `/`

- [.gitignore](.gitignore)
- [config.example.php](config.example.php)

## `public`

- [public/connexion.php](public/connexion.php)
- [public/creer.php](public/creer.php)
- [public/deconnexion.php](public/deconnexion.php)
- [public/index.php](public/index.php)
- [public/modifier.php](public/modifier.php)
- [public/supprimer.php](public/supprimer.php)

## `sql`

- [sql/schema.sql](sql/schema.sql)

## `src`

- [src/Auth.php](src/Auth.php)
- [src/CsrfHelper.php](src/CsrfHelper.php)
- [src/Database.php](src/Database.php)
- [src/TacheRepository.php](src/TacheRepository.php)
- [src/seed.php](src/seed.php)

---

Back to the [project README](README.en.md).
